...
- Switch provider: a TTP can express how a switch family can be easily controlled through OpenFlow
  • Example: Broadcom’s OF-DPA
- App or controller provider: TTPs can describe the switch behavior needed for a particular use case (i.e. a particular device role in a network architecture)
...
- Mixed group: TTPs can be created by a standards group (e.g., an ONF working group) to describe requirements for a particular solution
...
##<a name=""></a>What controllers will support TTPs?
